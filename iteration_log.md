...
## 2026-08-10 01:42 CST - Unified settings and privacy controls

### 本轮观察

- 当前功能分支比 `main` 超前 37 个提交，旧 PR 已合并但后续工作尚无开放 PR。
- 外观仍依赖前端 `localStorage`，无法可靠承载传感器、鼠标穿透和开机启动等系统级设置。
- 生产传感器读取失败时会回退到模拟数据，可能让桌宠表现出并不存在的系统活动。

### 本轮选择

先建立 Desktop Pet 0.2 的统一设置基础，再继续做状态和测试加固。设置由 Rust 持久化并广播，前端只负责展示和提交部分更新。

### 修改内容

- 新增版本化应用设置、部分更新、跨窗口事件和旧外观存储迁移。
- 设置窗口增加安静模式、三个检测开关、鼠标穿透、开机启动、隐私说明和全量恢复默认。
- 托盘增加安静模式和鼠标穿透恢复入口，菜单文案会随状态变化。
- 接入官方 Tauri autostart 插件；设置窗口扩大为可滚动布局。
- 传感器关闭或暂停时返回不可用值，不再使用生产模拟数据兜底。

### 验证结果

- `npm run check` 通过。
- Tauri `build --no-bundle` 通过。
- 440x620 浏览器预览无横向溢出，全部控制项可访问，开关交互无控制台错误。
- 基线 Draft PR #4 已创建，Windows CI 构建通过。
- 本机已有另一份同 bundle id 的桌宠实例运行，真实 Tauri UI 自动化无法可靠区分新旧窗口；本轮未关闭用户已有实例。

### 下一轮建议

- 为设置读写和心情策略增加自动化测试。
- 加固损坏设置恢复、跨显示器窗口位置和持续过载判定。
- 在 Windows 真机上验收开机启动、鼠标穿透恢复和传感器开关。

## 2026-08-10 01:52 CST - Reliability and automated test gates

### 本轮观察

- 设置文件解析失败时只静默使用默认值，用户无法判断原设置是否生效。
- 已保存的窗口位置只校验坐标范围，显示器移除后仍可能把桌宠恢复到不可见区域。
- 过载状态会被单次高 CPU 或高速打字立即触发，与规划中的“持续 10 秒”不一致。
- 本地检查和 Windows CI 尚未运行自动化测试。

### 修改内容

- 设置损坏时保留原文件、临时使用默认值，并在设置页显示可修复提示；用户保存任意设置后清除提示。
- 恢复窗口位置前检查桌宠中心点是否落在任一当前显示器内，兼容负坐标和多显示器布局；无效时居中。
- 高 CPU 或高速打字必须连续保持 10 秒才进入过载，信号下降或进入困倦状态会重置计时。
- 托盘安静模式与鼠标穿透菜单文案会随当前状态变化。
- 新增 Vitest 和 Rust 单元测试，并把前端测试、Rust 测试接入 `npm run check` 与 Windows CI。
- 测试过程中修复了关闭过载判定时高速打字仍可能触发过载的运算优先级问题。

### 验证结果

- 前端 3 个测试文件、16 个测试通过。
- Rust 12 个单元测试通过。
- 前端生产构建、Cargo fmt/check/clippy 全部通过。
...
- 发布流水线新增隔离的 NSIS 静默安装/卸载测试，覆盖主程序、卸载器、HKCU 卸载项、开始菜单快捷方式和卸载清理。
- 隔离安装/卸载测试已在 Windows runner 通过；继续增加已安装应用 5 秒启动存活检查，捕获安装后立即崩溃。
- Windows Build `31330203795` 完整通过：应用从隔离安装目录启动并存活 5 秒，随后精确停止该进程，静默卸载及残留检查通过。
- 自动化能够覆盖的 Windows 构建、安装、启动和卸载风险已基本穷尽；剩余门禁是可见 UI、托盘、重启、传感器、多显示器和睡眠唤醒的真实 Windows 人工验收。
